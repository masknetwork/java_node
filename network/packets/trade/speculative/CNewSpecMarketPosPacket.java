// Author : Vlad Cristian
// Contact : vcris@gmx.com

package wallet.network.packets.trade.speculative;

import wallet.kernel.UTILS;
import wallet.network.CResult;
import wallet.network.packets.CBroadcastPacket;
import wallet.network.packets.blocks.CBlockPayload;
import wallet.network.packets.trans.CFeePayload;

public class CNewSpecMarketPosPacket extends CBroadcastPacket
{
    public CNewSpecMarketPosPacket(String fee_adr,
                                   String adr, 
		                   long mktID, 
				   String tip, 
				   String ex_type,
				   double open, 
				   double sl, 
				   double tp, 
				   long leverage, 
				   double qty,
                                   String packet_sign,
                                   String payload_sign) throws Exception
    {
          super("ID_NEW_SPEC_MARKET_POS_PACKET");
	  
	  // Builds the payload class
	  CNewSpecMarketPosPayload dec_payload=new CNewSpecMarketPosPayload(adr, 
		                                                           mktID, 
				                                           tip, 
				                                           ex_type,
				                                           open, 
				                                           sl, 
				                                           tp, 
				                                           leverage, 
				                                           qty);
					
	  // Build the payload
	  this.payload=UTILS.SERIAL.serialize(dec_payload);
					
          // Network fee
	  fee=new CFeePayload(fee_adr);
			   
	   // Sign packet
	   this.sign(payload_sign);
	}
		
        // Check 
	public CResult check(CBlockPayload block) throws Exception
	{
	    // Super class
	    CResult res=super.check(block);
	    if (res.passed==false) return res;
		   	
	    // Check type
	    if (!this.tip.equals("ID_NEW_SPEC_MARKET_POS_PACKET"))
	       return new CResult(false, "Invalid packet type", "CNewSpecMarketPosPacket", 42);
		   
	    // Deserialize transaction data
	    CNewSpecMarketPosPayload dec_payload=(CNewSpecMarketPosPayload) UTILS.SERIAL.deserialize(payload);
		   
	    // Check payload
	    res=dec_payload.check(block);
            if (!res.passed) return res;
            
            // Check fee
            if (this.fee.amount<0.0001)
               return new CResult(false, "Invalid price", "CNewSpecMarketPosPacket", 42);
            
	    // Return 
	    return new CResult(true, "Ok", "CNewSpecMarketPosPacket", 74);
	}
		   
	public CResult commit(CBlockPayload block) throws Exception
	{
	    // Superclass
            CResult res=super.commit(block);
	    if (res.passed==false) return res;
		   	  
	    // Deserialize transaction data
	    CNewSpecMarketPosPayload ass_payload=(CNewSpecMarketPosPayload) UTILS.SERIAL.deserialize(payload);

	    // Commit payload
	    res=ass_payload.commit(block);	  
	    if (res.passed==false) return res;
	    		  
	    // Return 
            return new CResult(true, "Ok", "CNewSpecMarketPosPacket", 9);
      }
}
