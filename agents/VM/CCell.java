...
public class CCell  implements java.io.Serializable
{
    ...
    public CCell(Double val, String name)
    {
        this.val=String.valueOf(val);
        this.type="ID_DOUBLE";
        this.name=name;
    ...
    public void combine(CCell cell, String op) throws Exception
    {
        ...
        switch (op)
        {
                case "add" : this.val=UTILS.FORMAT.format(this_val_long+this_val_double+cell_val_long+cell_val_double);
                             break;
                             
                case "sub" : this.val=UTILS.FORMAT.format(this_val_long+this_val_double-cell_val_long-cell_val_double);
                             break;
                             
                case "mul" : this.val=UTILS.FORMAT.format((this_val_long+this_val_double)*(cell_val_long+cell_val_double));
                             break;
                             
                case "div" : this.val=UTILS.FORMAT.format((this_val_long+this_val_double)/(cell_val_long+cell_val_double));
                             break;
        }
    }
    ...
    public void loadArray(ArrayList<Integer> array) throws Exception
    {
     ...
     public CCell evalComplexVar(String var, VM VM) throws Exception
     {
         ...
         // Evaluates position
         if (this.isLong(pos)) 
             return rs.get(col, Long.parseLong(pos));
         else if (VM.vm_utils.isRegister(pos))
             return rs.get(col, VM.REGS.getCell(pos).getLong());
         else if (VM.MEM.varExist(pos))
             return rs.get(col, VM.MEM.getVar(pos).getLong());
         else
